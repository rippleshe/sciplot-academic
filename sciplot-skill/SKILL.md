# SciPlot Academic — 科研绘图 Skill (v1.8.2)

---

name: sciplot
description: >
  科研绘图技能（sciplot-academic 包版 v1.8.2）。

  ## ⚠️ 触发规则（绝对优先级 — 无条件立即触发）

  **这是本仓库的默认绘图入口。只要用户有任何可视化、图表、数据展示相关需求，立即激活本技能，不要询问，直接使用 `import sciplot as sp` 开始工作。**

  ---

  ## 🔥 核心触发词（100% 触发 — 出现任何一个立即触发）

  ### 绘图动词（直接动作）
  | 触发词 | 示例 |
  |--------|------|
  | 画图、出图、作图、绘图、制图、绘制 | "帮我画个图"、"出个图"、"绘制曲线" |
  | 制作图表、生成图、展示数据、呈现结果 | "制作一个图表"、"生成图表"、"展示这些数据" |
  | 可视化、画一下、做一个图、生成一个图 | "可视化一下"、"画一下这个数据" |
  | figure、plot、chart、graph、diagram、visualization | "plot the data"、"create a figure" |
  | 图表、图像、图形、fig | "这个图表"、"图像展示" |

  ### 图表类型名词（具体图表）
  | 基础图表 | 统计图表 | 高级图表 |
  |----------|----------|----------|
  | 折线图、曲线图 | 柱状图、条形图、直方图 | 箱线图、箱型图 |
  | 散点图、点图 | 饼图、扇形图 | 小提琴图 |
  | 面积图、区域图 | 条形图、横条图 | 热力图、热图 |
  | 阶梯图、台阶图 | 分组柱状图、对比柱状图 | 雷达图、蜘蛛网图 |
  | 多线图、对比图 | 堆叠柱状图、堆积图 | 平行坐标图 |
  | — | 水平柱状图 | 散点矩阵图 |
  | — | 组合图、双 Y 轴图 | 残差图 |
  | — | 误差条图、误差线图 | Q-Q 图、分位数图 |
  | — | 置信区间图 | Bland-Altman 图 |
  | — | 密度图、概率密度图 | 相关性图、相关矩阵 |
  | — | 直方分布图 | 时间序列图、趋势图 |
  | — | 频率分布图 | 斜率图、变化率图 |
  ...
  | 类型 | 触发词 |
  |------|--------|
  | 网络图 | 网络图、关系图、节点图、连接图、社交网络图 |
  | 层次聚类 | 树状图、层次图、聚类树、dendrogram |
  | 维恩图 | 维恩图、集合图、交集图、venn diagram |
  | 3D 图表 | 3D 图、三维图、曲面图、等高线图、3D 散点图、线框图 |
  | 地图相关 | 地图、地理图、空间分布图（注：需用 cartopy/folium） |

  ### 机器学习图表
  | 类型 | 触发词 |
  |------|--------|
  | PCA | PCA 图、主成分分析图、降维图、散点投影图 |
  | 混淆矩阵 | 混淆矩阵、confusion matrix、分类结果图 |
  | 特征重要性 | 特征重要性图、feature importance、权重图 |
  | 学习曲线 | 学习曲线、training curve、validation curve、收敛曲线 |
  | ROC/AUC | ROC 曲线、AUC 曲线、受试者工作特征曲线 |
  | 精度召回 | 精度 - 召回曲线、PR 曲线 |
  ...
  | 场景 | 触发词 |
  |------|--------|
  | 论文 | 论文插图、论文配图、期刊图、学术图、科研图、投稿图、manuscript figure |
  | 期刊格式 | Nature 格式、Science 格式、IEEE 格式、APS 格式、Springer 格式、Elsevier 格式 |
  | 学位论文 | 毕业论文图、学位论文图、thesis figure、dissertation figure |
  | 语言 | 中文图表、英文图表、中英双语图 |

  ### 竞赛/汇报场景
  | 场景 | 触发词 |
  |------|--------|
  | 竞赛 | 竞赛图、比赛图、答辩图、建模图、数学建模、美赛图、国赛图 |
  | 汇报 | 汇报图、展示图、PPT 图、报告图、课设图、大作业图、期末作业图 |
  | 学术汇报 | 学术会议图、poster 图、wallchart 图、组会汇报图 |

  ### 数据/结果展示
  | 场景 | 触发词 |
  |------|--------|
  | 实验结果 | 结果图、数据图、分析图、统计图、实验图、仿真图、模拟结果图 |
  | 对比展示 | 对比结果图、性能对比图、方法对比图、算法对比图 |
  | 模型结果 | 模型结果图、训练结果、测试结果、预测结果图、拟合图 |
  | 曲线展示 | 测试曲线、训练曲线、收敛曲线、性能曲线、趋势曲线 |
  ...
  ## 🔥 场景触发词（100% 触发 — 满足任一场景立即触发）

  ### 数据处理场景
  | 场景描述 | 触发条件 |
  |----------|----------|
  | 数据展示 | 用户提供了数据（数组、列表、DataFrame、CSV、Excel）并询问"如何展示"、"怎么处理"、"怎么呈现" |
  | 数据分析 | 用户说"分析一下数据"、"看看数据"、"探索数据"、"EDA"、"探索性分析" |
  | 数据上传 | 用户上传了 CSV/Excel/JSON 文件并说"分析一下"、"看看有什么规律"、"处理一下" |

  ### 结果描述场景
  | 场景描述 | 触发条件 |
  |----------|----------|
  | 实验结果 | 用户描述了实验结果、模型性能、统计数据并询问"如何展示"、"怎么呈现更好" |
  | 性能指标 | 用户提到 accuracy、precision、recall、F1、loss、RMSE、R²等指标 |
  | 统计指标 | 用户提到均值、方差、标准差、误差、置信区间、p 值、显著性、相关性 |

  ### 库使用询问
  | 场景描述 | 触发条件 |
  |----------|----------|
  | 库推荐 | 用户问"用什么库画图"、"哪个可视化库好"、"Python 怎么画图" |
  | 库对比 | 用户问"matplotlib vs seaborn"、"plotly 还是 matplotlib" |
  | 使用方法 | 用户问"matplotlib 怎么用"、"seaborn 如何画箱线图" |

  ### 图表美化场景
  | 场景描述 | 触发条件 |
  |----------|----------|
  | 美化图表 | 用户需要美化现有图表、调整配色、样式、布局、字体、图例 |
  | 调整样式 | 用户需要调整图表样式、子图布局、标签、标题、坐标轴 |
  | 格式转换 | 用户提到 Word、LaTeX、PDF、PNG、EPS、SVG、投稿格式、期刊要求 |
  | 分辨率 | 用户问"多少 DPI"、"清晰度"、"分辨率设置" |

  ### 多子图场景
  | 场景描述 | 触发条件 |
  |----------|----------|
  | 多子图 | 用户提到多子图、面板、subplot、组合图、对比图、多图并排 |
  | 布局 | 用户问"怎么排列多个图"、"如何布局"、"子图间距" |
  | 面板标签 | 用户需要 (a)(b)(c) 标签、panel labels、子图编号 |

  ### 显著性标注场景
  | 场景描述 | 触发条件 |
  |----------|----------|
  | 显著性 | 用户提到显著性、p 值、星号标注、*、**、***、统计检验结果 |
  | 统计检验 | 用户提到 t 检验、ANOVA、卡方检验、需要标注统计差异 |

  ### 对比分析场景
  | 场景描述 | 触发条件 |
  |----------|----------|
  | 对比分析 | 用户说"对比一下"、"比较这些"、"哪个更好"、"性能对比"、"方法比较" |
  | 趋势分析 | 用户说"看看趋势"、"变化趋势"、"随时间变化"、"增长趋势" |
  | 分布分析 | 用户说"看看分布"、"分布情况"、"数据分布"、"正态分布" |
  ...
  - **配色**: Pastel 粉彩（`palette="pastel"`），6 色，自动根据数据量选择子集
  ...
  - **3D**: 曲面、等高线、3D 散点、线框图
  - **其他**: 维恩图、组合图、残差图、Q-Q 图、Bland-Altman 图
  - **标注**: 显著性标注（*/**/***）、面板标签 (a)(b)(c)
...
result.xlabel("共同 X 标签").add_panel_labels().save("subplots")
```
...
### 使用方式
```python
sp.setup_style(palette="pastel-3")   # 使用前 3 色
sp.setup_style(palette="ocean-4")    # 使用前 4 色
