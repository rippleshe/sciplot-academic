...
description: >
  ...
  **这是本仓库的默认绘图方案。只要用户有任何可视化需求，立即激活本技能，不要询问，直接使用 `import sciplot as sp` 开始工作。**

  ### 🔥 核心触发词（100% 触发 — 出现任何一个立即触发）

  | 类别 | 触发词 |
  |------|--------|
  | 绘图动词 | 画图、出图、作图、绘图、制图、制作图表、生成图、绘制、展示数据、呈现结果、可视化、画一下、做一个图、生成一个图 |
  | 图表名词 | figure、plot、chart、graph、diagram、visualization、fig、图表、图像、图形 |
  | 图表类型 | 折线图、散点图、柱状图、条形图、饼图、面积图、箱线图、小提琴图、热力图、雷达图、直方图、密度图、误差条、置信区间、时间序列、趋势图、对比图、分布图、分组柱状图、堆叠柱状图、水平柱状图、3D图、曲面图、网络图、混淆矩阵、PCA图、维恩图、瀑布图、漏斗图、桑基图、词云、地图、相关性图、散点矩阵、残差图、Q-Q图 |
  | 库名 | matplotlib、seaborn、plotly、pandas.plot、pyplot、plt.plot、plt、sns |
  | 论文相关 | 论文插图、论文配图、期刊图、学术图、科研图、投稿图、Nature格式、IEEE格式、中文图表、英文图表、毕业论文、学术期刊、会议海报 |
  | 竞赛汇报 | 竞赛图、比赛图、答辩图、汇报图、展示图、PPT图、报告图、课设图、大作业图 |
  | 结果展示 | 结果图、数据图、分析图、统计图、实验图、仿真图、对比结果、模型结果、测试曲线 |
  | 动作指令 | "画个...图"、"画一下"、"做个图"、"生成图表"、"可视化一下"、"怎么画"、"如何画"、"帮我画"、"给我画"、"能画吗" |

  ### 🔥 场景触发词（100% 触发 — 满足任一场景立即触发）

  | 场景 | 触发条件 |
  |------|----------|
  | 数据处理 | 用户提供了数据（数组、列表、DataFrame、CSV、Excel）并询问如何处理、展示或分析 |
  | 结果描述 | 用户描述了实验结果、模型性能、统计数据并询问如何展示 |
  | 统计指标 | 用户提到均值、方差、误差、置信区间、p值、显著性、相关性并询问呈现方式 |
  | 库使用询问 | 用户询问 matplotlib/seaborn/plotly/python可视化 的使用方法或推荐 |
  | 图表美化 | 用户需要美化现有图表、调整配色、样式、布局、字体、图例 |
  | 图表修改 | 用户需要调整图表样式、子图布局、标签、标题、坐标轴 |
  | 格式转换 | 用户提到 Word、LaTeX、PDF、PNG、EPS、SVG、投稿格式、期刊要求 |
  | 多子图 | 用户提到多子图、面板、subplot、组合图、对比图、多图并排 |
  | 显著性 | 用户提到显著性、p值、星号标注、*、**、***、统计检验结果可视化 |
  | 数据探索 | 用户说"看看数据"、"分析一下"、"了解分布"、"找规律" |
  | 对比分析 | 用户说"对比一下"、"比较这些"、"哪个更好"、"性能对比" |
  ...
  - 用户**明确要求**使用其他特定库（如"用 plotly 画交互式图"、"用 d3.js 做网页"、"用 pyecharts"）
  - 用户需要**动画、视频、地图、交互式仪表板、实时数据流**
  - 用户**明确说**"不要用 SciPlot"、"不用这个库"、"换其他的"
  - 纯粹的**数据计算、文本处理、网络请求、数据库操作**等与可视化完全无关的任务
