...
class TestCoreFunctionsReturnTypes:
    ...
    def test_new_figure_returns_tuple(self, cleanup_figures):
    ...
    def test_save_multiple_formats_returns_multiple_paths(self, temp_dir, cleanup_figures):
        """save 多格式应该返回多个 Path"""
        fig, ax = sp.plot([1, 2], [1, 2])
        output_path = temp_dir / "test"
...
class TestBasicChartsReturnTypes:
    ...
    def test_plot_returns_tuple(self, test_data, cleanup_figures):
        """plot 应该返回 (Figure, Axes)"""
    ...
    def test_plot_line_returns_tuple(self, test_data, cleanup_figures):
        """plot_line 应该返回 (Figure, Axes)"""
        result = sp.plot_line(test_data["x"], test_data["y"])
        assert isinstance(result, tuple)
        assert isinstance(result[0], Figure)
        assert isinstance(result[1], Axes)
        
    def test_plot_multi_returns_tuple(self, test_data, cleanup_figures):
        """plot_multi 应该返回 (Figure, Axes)"""
        result = sp.plot_multi(test_data["x"], [test_data["y"], test_data["y2"]])
        assert isinstance(result, tuple)
        assert isinstance(result[0], Figure)
        assert isinstance(result[1], Axes)
        
    def test_plot_scatter_returns_tuple(self, test_data, cleanup_figures):
        """plot_scatter 应该返回 (Figure, Axes)"""
        result = sp.plot_scatter(test_data["x"], test_data["y"])
        assert isinstance(result, tuple)
        assert isinstance(result[0], Figure)
        assert isinstance(result[1], Axes)
        
    def test_plot_bar_returns_tuple(self, test_data, cleanup_figures):
        """plot_bar 应该返回 (Figure, Axes)"""
        result = sp.plot_bar(test_data["categories"], test_data["values"])
        assert isinstance(result, tuple)
        assert isinstance(result[0], Figure)
        assert isinstance(result[1], Axes)
        
    def test_plot_grouped_bar_returns_tuple(self, cleanup_figures):
        """plot_grouped_bar 应该返回 (Figure, Axes)"""
    ...
    def test_plot_stacked_bar_returns_tuple(self, cleanup_figures):
        """plot_stacked_bar 应该返回 (Figure, Axes)"""
    ...
    def test_plot_horizontal_bar_returns_tuple(self, test_data, cleanup_figures):
        """plot_horizontal_bar 应该返回 (Figure, Axes)"""
        result = sp.plot_horizontal_bar(test_data["categories"], test_data["values"])
        assert isinstance(result, tuple)
        assert isinstance(result[0], Figure)
        assert isinstance(result[1], Axes)
        
    def test_plot_area_returns_tuple(self, test_data, cleanup_figures):
        """plot_area 应该返回 (Figure, Axes)"""
        result = sp.plot_area(test_data["x"], test_data["y"])
        assert isinstance(result, tuple)
        assert isinstance(result[0], Figure)
        assert isinstance(result[1], Axes)
        
    def test_plot_multi_area_returns_tuple(self, test_data, cleanup_figures):
        """plot_multi_area 应该返回 (Figure, Axes)"""
        result = sp.plot_multi_area(test_data["x"], [test_data["y"], test_data["y2"]])
        assert isinstance(result, tuple)
        assert isinstance(result[0], Figure)
        assert isinstance(result[1], Axes)
        
    def test_plot_step_returns_tuple(self, test_data, cleanup_figures):
        """plot_step 应该返回 (Figure, Axes)"""
        result = sp.plot_step(test_data["x"], test_data["y"])
        assert isinstance(result, tuple)
        assert isinstance(result[0], Figure)
        assert isinstance(result[1], Axes)
        
    def test_plot_errorbar_returns_tuple(self, test_data, cleanup_figures):
        """plot_errorbar 应该返回 (Figure, Axes)"""
    ...
    def test_plot_confidence_returns_tuple(self, test_data, cleanup_figures):
        """plot_confidence 应该返回 (Figure, Axes)"""
...
class TestDistributionChartsReturnTypes:
    ...
    def test_plot_box_returns_tuple(self, test_data, cleanup_figures):
        """plot_box 应该返回 (Figure, Axes)"""
        result = sp.plot_box(test_data["data_list"])
        assert isinstance(result, tuple)
        assert isinstance(result[0], Figure)
        assert isinstance(result[1], Axes)
        
    def test_plot_violin_returns_tuple(self, test_data, cleanup_figures):
        """plot_violin 应该返回 (Figure, Axes)"""
        result = sp.plot_violin(test_data["data_list"])
        assert isinstance(result, tuple)
        assert isinstance(result[0], Figure)
        assert isinstance(result[1], Axes)
        
    def test_plot_histogram_returns_tuple(self, cleanup_figures):
        """plot_histogram 应该返回 (Figure, Axes)"""
    ...
    def test_plot_heatmap_returns_tuple(self, test_data, cleanup_figures):
        """plot_heatmap 应该返回 (Figure, Axes)"""
...
class TestLayoutFunctionsReturnTypes:
    ...
    def test_create_subplots_returns_tuple(self, cleanup_figures):
    ...
    def test_paper_subplots_returns_tuple(self, cleanup_figures):
    ...
    def test_create_gridspec_returns_tuple(self, cleanup_figures):
...
class TestReturnValueConsistency:
    ...
    def test_all_plot_functions_return_same_structure(self, test_data, cleanup_figures):
        """所有绘图函数应该返回相同的 (Figure, Axes) 结构"""
        functions = [
        ...
        for func in functions:
            result = func()
            assert isinstance(result, tuple), f"{func.__name__} 应该返回元组"
            assert len(result) == 2, f"{func.__name__} 应该返回2个元素"
            assert isinstance(result[0], Figure), f"{func.__name__} 第一个元素应该是 Figure"
            assert isinstance(result[1], Axes), f"{func.__name__} 第二个元素应该是 Axes"
