...
- colorbar 是辅助信息而不是第二主轴：outline、ticks 和字号应比主坐标轴略弱，尺寸/间距保持克制；不要让色条抢占数据区或成为视觉焦点。
- 单栏窄画布中的大型长图例不能横穿数据区：先按真实 renderer bbox 判断占用；确实过密时优先移到图外下方并控制列数，保持正文宽度不被横向撑大。
